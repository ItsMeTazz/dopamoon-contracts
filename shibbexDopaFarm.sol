...
contract ShibbexDopaFarm is ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    ...
    function setNotifier(address _rewardsToken, address _notifier)
        external
        onlyOwner
    {
        rewardData[_rewardsToken].notifier = _notifier;
    ...
    /// @notice stake the LP into the multirewarder
        ...
        updateReward(msg.sender)
    {
        ...
        uint256 _tax;
        uint256 _send;
        (_tax, _send) = _fee(amount);
        require(_tax.add(_send) == amount, "Fee calculation error");
        _totalSupply = _totalSupply.add(_send);
    ...
    /// @notice incur a fee on deposit
    function _fee(uint256 _amount) internal view returns (uint256, uint256) {
        uint256 _twoPointZero = ((_amount * depositFee) / 10000);
        uint256 _newAmount = _amount - _twoPointZero;
        return (_twoPointZero, _newAmount);
    }

    function adjustFee(uint256 _newDepositFee) external onlyOwner {
        require(_newDepositFee <= 500, "!valid"); //max 5%
